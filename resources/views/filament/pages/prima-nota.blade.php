<x-filament-panels::page>
    <x-filament::section>
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:gap-4">
            ...
            <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300" for="bankAccountId">Conto</label>
                <select id="bankAccountId" wire:model.live="bankAccountId"
                    class="fi-input block w-full rounded-lg border-gray-300 dark:border-white/10 dark:bg-white/5 dark:text-white">
                ...
                <thead>
                    <tr class="border-b border-gray-200 dark:border-white/10 text-left">
                        @foreach ($table['headings'] as $i => $heading)
                            <th class="px-3 py-2 font-semibold whitespace-nowrap {{ in_array($i, [4, 5, 6]) ? 'text-right' : '' }}">
                                {{ $heading }}
                            </th>
                        @endforeach
                    </tr>
                ...
                <tbody>
                    @forelse ($table['rows'] as $row)
                        <tr class="border-b border-gray-100 dark:border-white/5
                            {{ $row['kind'] === 'subtotal' ? 'font-semibold bg-gray-50 dark:bg-white/5' : '' }}">
                            @foreach ($row['cells'] as $i => $cell)
                                <td class="px-3 py-2 align-top {{ in_array($i, [4, 5, 6]) ? 'text-right whitespace-nowrap tabular-nums' : '' }}">
                                    @if (in_array($i, [4, 5, 6]) && is_numeric($cell) && $cell !== '')
