# TrackFlow

**TrackFlow** is a streamlined, professional web application designed for tracking billable hours and business expenses. Built for freelancers and consultants, it offers a clean, distraction-free interface to manage daily activities and financial logs with ease.

---

## Key Features

-   **Time Tracking:** Quickly log work sessions and associate them with specific clients.
-   **Expense Management:** Track out-of-pocket costs, materials, and travel expenses.
-   **Minimalist Dashboard:** A clean overview of total hours worked and expenses incurred.
-   **Mobile-First Design:** Fully responsive layout optimized for quick entries on the go.

## Documentation

-   [`docs/api-abbonamenti.md`](docs/api-abbonamenti.md) — inbound API that turns a
    collected subscription payment into a draft invoice (called by personal-ticketing).

## Tech Stack (TALL Stack)

This project leverages the power of the TALL stack for a modern, reactive development experience:

-   **Laravel** – Robust PHP framework for backend logic and API management.
-   **Livewire** – Dynamic interfaces without leaving the comfort of PHP.
-   **Alpine.js** – Lightweight JavaScript for client-side interactivity.
-   **Tailwind CSS** – Utility-first CSS framework for rapid UI development.
